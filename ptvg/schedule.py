...
"""ptvg personal tv guide schedules module."""
    ...
    try:
        ...
        if testDictKeys(cdata, reqkeys):
            if int(cdata["code"]) == 0:
                ...
                sql += "values (?, ?, ?, ?, ?)"
                lmts = ptvg.getTimeStamp(cdata["lastModified"])
                dts = ptvg.getTimeStamp(date, dtformat="%Y-%m-%d")
                ...
                for prog in sched["programs"]:
                    ...
                    sdb.insertSql(
                        ...
                        [
                            ...
                            prog["md5"],
                            chan,
                            ptvg.getTimeStamp(prog["airDateTime"]),
                            int(prog["duration"]),
                        ],
                    )
